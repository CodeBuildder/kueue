...
func TestScheduleForTAS(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"workload gets scheduled when another admitted workload has a completed podset that released its topology domain": {
			nodes: []corev1.Node{
				*testingnode.MakeNode("x1").
					Label("tas-node", "true").
					Label(corev1.LabelHostname, "x1").
		...
		"workload does not get scheduled when ReclaimablePods is disabled even if another admitted workload has a completed podset": {
			nodes: []corev1.Node{
				*testingnode.MakeNode("x1").
					Label("tas-node", "true").
					Label(corev1.LabelHostname, "x1").
			...
			eventCmpOpts: cmp.Options{eventIgnoreMessage},
			wantEvents: []utiltesting.EventRecord{
				utiltesting.MakeEventRecord("default", "foo", kueue.WorkloadQuotaReservedReasonWaitingForQuota, "Warning").Obj(),
			},
			featureGates: map[featuregate.Feature]bool{
...
func TestScheduleForTASPreemption(t *testing.T) {
	...
	cases := map[string]tasScheduleTestCase{
		...
		"workload with unhealthyNode needing preemption does not force-preempt others and stays pending": {
			...
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("low-prio-admitted", "default").
					UID("low-prio-admitted-uid").
					Queue("tas-main").
					Priority(1).
					ReserveQuotaAt(
						utiltestingapi.MakeAdmission("tas-main").
					...
					PodSets(*utiltestingapi.MakePodSet("one", 1).
						...
						Obj()).
					Obj(),
				*utiltestingapi.MakeWorkload("marked-needs-preempt", "default").
					UID("marked-needs-preempt-uid").
					UnhealthyNodes("x1").
					Queue("tas-main").
					Priority(3).
					PodSets(*utiltestingapi.MakePodSet("one", 1).
			...
			// The admitted low-priority workload must not be touched: forcing its
			// preemption here would mean we treated a failed-node replacement like
			// an ordinary capacity preemption, which is exactly the bug.
			wantWorkloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("low-prio-admitted", "default").
					UID("low-prio-admitted-uid").
					Queue("tas-main").
			...
			wantInadmissibleLeft: map[kueue.ClusterQueueReference][]workload.Reference{
				"tas-main": {"default/marked-needs-preempt"},
			},
			eventCmpOpts: cmp.Options{
				eventIgnoreMessage,
				cmpopts.IgnoreFields(utiltesting.EventRecord{}, "Reason"),
			},
			wantEvents: []utiltesting.EventRecord{
				utiltesting.MakeEventRecord("default", "marked-needs-preempt", "", corev1.EventTypeWarning).Obj(),
			},
		},
		// A second regression case for #15337, requested during review of
		// #15536: TASFailedNodeReplacementFailFast disabled, quota
		// exhausted, but a node genuinely free for the replacement. The
		// workload's own quota need looks like it requires preemption
		// (low-prio-admitted already holds all of it), but the failed
		// node's replacement domain (y2) is free, so no one should
		// actually be preempted for it.
		"workload with unhealthyNode needing preemption finds a free node without preempting anyone": {
			nodes: []corev1.Node{
				*testingnode.MakeNode("x1").
					Label("tas-node", "true").
					Label(corev1.LabelHostname, "x1").
			...
			// low-prio-admitted must stay untouched: the free node (y2)
			// means marked-needs-preempt never actually needed anyone
			// else's capacity.
			wantWorkloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("low-prio-admitted", "default").
					UID("low-prio-admitted-uid").
